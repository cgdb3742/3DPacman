...
void TIMER::update_horloge()
{
	...
	tmp               = timer_get_ms();
	dt      = tmp - last_global_timer;
	last_global_timer = tmp;


	local_timer_40_Hz += dt;
	local_timer_25_Hz += dt;
	local_timer_20_Hz += dt;
	local_timer_16_Hz += dt;
	local_timer_10_Hz += dt;
	local_timer_05_Hz += dt;
	local_timer_02_Hz += dt;
	local_timer_01s   += dt;
	local_timer_02s   += dt;
	local_timer_10s   += dt;
	local_timer_30s   += dt;
